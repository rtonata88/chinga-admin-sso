...
export default function Teams({ teams, filters }: Props) {
    ...
    const handleSearch = () => {
        router.get('/platform/fantasy/teams', { search: search || undefined }, { preserveState: true });
    };
    ...
    const handleSubmit = () => {
        ...
        if (editingTeam) {
            router.put(`/platform/fantasy/teams/${editingTeam.uuid}`, data, {
                onSuccess: () => { setDialogOpen(false); setSaving(false); },
                onError: () => setSaving(false),
            });
        } else {
            router.post('/platform/fantasy/teams', data, {
                onSuccess: () => { setDialogOpen(false); setSaving(false); },
                onError: () => setSaving(false),
            });
        }
    ...
    const handleDelete = () => {
        ...
        router.delete(`/platform/fantasy/teams/${deletingTeam.uuid}`, {
                            ...
                            >
                                ...
                                <div className="flex gap-2">
                                    <Button
                                        ...
                                        onClick={() => router.get('/platform/fantasy/teams', { ...filters, page: teams.current_page - 1 }, { preserveState: true })}
                                    />
                                    <Button
                                        label="Next"
